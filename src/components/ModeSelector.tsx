  ...
  return (
    <div className="card" role="region" aria-label="Mode selection" style={{ padding: 0, overflow: 'hidden' }}>
      <div style={{ display: 'grid', gridTemplateRows: '1fr 1fr 1fr', height: 'calc(100vh - 180px)' }}>
        ...
        <button className="btn" style={{ borderRadius: 0, fontSize: 18 }} disabled={!state.savedFinalList || state.savedFinalList.length === 0} onClick={() => { dispatch({ type: 'showSavedFinal' }); }}>Load previous list</button>
      </div>
    </div>
  );
}
